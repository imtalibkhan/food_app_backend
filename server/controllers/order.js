...
export const placeOrder = asyncError(async (req, res, next) => {
  ...
  res.status(201).json({
    success: true,
    message: "Order Placed Successfully via cash on Delevery",
  });
});
...
export const paymentVerification = asyncError(async (req, res, next) => {
  ...
  if(isAuthentic){
    ...
    res.status(201).json({
      success: true,
      message: `Order Plcae succesfully.Payment ID: ${payment._id}`,
...
export const processOrder = asyncError(async (req, res, next) => {
  ...
  if (order.orderStatus === "Preparing") order.orderStatus = "Shipped";
    ...
    return next(new ErrorHandler("Food Already Delevered", 400));
  }
  await order.save();
